...
describe('isDiagnosticAutoDismissed — amount edits', () => {
  ...
  it('dismisses importMismatches when all source gaps are cleared', () => {
    ...
    const c = ctx({
      amounts: {
        ...
        necIncome: SOURCE_AMOUNTS.necIncome,
        necOnReturn: true,
      },
    })
  ...
  it('includes NEC silent omit in import mismatches at seed', () => {
    const rows = getOutstandingImportMismatches(SEED_AMOUNTS)
    const nec = rows.find(r => r.id === 'necIncome')
    expect(nec).toBeDefined()
    expect(nec?.returnValue).toBe('$0')
    expect(nec?.sourceValue).toBe('$24,000')
    expect(nec?.field).toBe('nec-box1')
    expect(nec?.tab).toBe('1099-necs')
  })
})
